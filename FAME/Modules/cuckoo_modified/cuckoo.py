...
class CuckooModifi(ProcessingModule):
    name = "cuckoo_modifi"
    ...
    def each_with_type(self, target, file_type):
        ...
        self.results['URL'] = "{}/tasks/report/{}/html".format(self.base_url, self.task_id)
 ...
 #           'tags': tag
    ...
    def submit_url(self, target_url, options):
        url = urljoin(self.base_url, '/tasks/create/url')
        options['url'] = target_url
        response = requests.post(url, data=options)
        self.task_id = response.json()['task_id']

    def wait_for_analysis(self):
        url = urljoin(self.base_url, '/tasks/view/{0}'.format(self.task_id))
    ...
    def store_report_summary(self):
        url = urljoin(self.base_url, '/tasks/report/{0}/html'.format(self.task_id))
        tmpdir = tempdir()
        filepath = urlretrieve(url, os.path.join(tmpdir, 'report.html'))[0]
        self.add_support_file('Report', filepath)

    def process_report(self):
        url = urljoin(self.web_base_url, '/api/tasks/get/iocs/{0}'.format(self.task_id))
    ...
    def get_pcap(self):
        url = urljoin(self.base_url, '/pcap/get/{0}'.format(self.task_id))
        response = requests.get(url, stream=True)

        self.register_response_as('pcap', response)

    def get_memory_dump(self):
        url = urljoin(self.base_url, '/memory/list/{0}'.format(self.task_id))
        response = requests.get(url, stream=True)
    ...
    def done(self):
       report = MISPmod.get_cuckoo_json()
       misp_report = MISPmod.create_misp_report(report)
       api = open('/home/fame/.donotopen','r').read()
       respond = MISPmod.post_report(api, misp_report)    
